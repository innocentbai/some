...
n_bins = 88                              # Number of frequency bins
mag_exp = 4                              # Magnitude Exponent
pre_post_max = 4                         # Pre- and post- samples for peak picking
cqt_threshold = -77                      # Threshold for CQT dB levels, all values below threshold are set to -120 dB
...
def generate_sine_midi_note(f0_info, sr, n_duration, round_to_sixtenth=True):
    ...
    note_duration = max(0.05*np.around(duration/2/0.05),0.05) # Round to 2 decimal places for music21 compatibility
    ...
    if f0==None:
        ...
        note = Note(librosa.midi_to_note(midi_note).replace('♯','#'), type=mm.secondsToDuration(max(note_duration,0.05)).type)
